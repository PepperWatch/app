    ...
    async decryptMessage(encryptedMessage) {
      return await new Promise((res,rej)=>{
        ...
        window.ethereum
          .request({
            method: 'eth_decrypt',
            params: [encryptedMessage, this.MetaMaskAddress], // you must have access to the specified account
          })
    ...
    async getEncryptionPublicKey() {
      return await new Promise((res,rej)=>{
        if (this.encryptionPublicKey) {
          return res(this.encryptionPublicKey);
        }

        window.ethereum
          .request({
            method: 'eth_getEncryptionPublicKey',
            params: [this.MetaMaskAddress], // you must have access to the specified account
          })
          .then((result) => {
